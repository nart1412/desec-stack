...
def test_redirects(protocol, hostname):
    ...
    response = get(f'{protocol}://{hostname}/', allow_redirects=False)
...
def test_checkip(server_ip, protocol, hostname):
        ...
        if not hostname.startswith('checkip.') and not hostname.startswith(f'checkip{ip_version}.'):
            with warnings.catch_warnings():
                ...
                with pytest.raises(exceptions.ConnectionError) as excinfo:
                    get(f'{protocol}://{hostname}/', allow_redirects=False, verify=False)
...
@pytest.mark.parametrize("protocol", ['http', 'https'])
def test_unknown_hosts(protocol, hostname):
    with warnings.catch_warnings():
        warnings.filterwarnings('ignore', category=InsecureRequestWarning)
        with pytest.raises(exceptions.ConnectionError) as excinfo:
            get(f'{protocol}://{hostname}/', allow_redirects=False, verify=False)
